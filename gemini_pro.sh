...
#
# - v3.5 更新日志 (彻底重构):
#   - [重大修复] 放弃所有单行压缩代码，采用标准格式，从根源解决所有语法错误。
#   - [重大修复] 重写日志和错误捕获逻辑，确保所有失败都有明确、完整的错误输出。
#   - [健壮性提升] 重写配额检查函数(check_quota)，使用更可靠的解析方式。
#   - [可读性] 全面优化代码格式和输出，提升可读性和用户体验。
#
# ==============================================================================
...
retry_with_backoff() {
    ...
    while (( attempt <= max_attempts )); do
        ...
        local output
        local error_msg
        output=$(eval "$cmd_str" 2> >(error_msg=$(cat); cat >&2))
        local exit_code=$?
...
show_progress() {
    ...
    local bar; printf -v bar '%*s' "$filled_len" ''; bar=${bar// /█}
    local empty; printf -v empty '%*s' "$((bar_len - filled_len))" ''; empty=${empty// /░}

    printf "\r%-80s" " "
    printf "\r[%s%s] %d%% (%d/%d) - %s" "$bar" "$empty" "$percent" "$completed" "$total" "$op_name"
}
...
task_create_key() {
    ...
    if [[ -z "$create_output" ]]; then return 1; fi
...
run_parallel() {
    ...
    if (( total_items == 0 )); then
        log "INFO" "在 '$description' 阶段无项目处理。"
        return
    fi
    ...
    local success_count
    success_count=$(wc -l < "$success_file" | xargs)
...
check_quota() {
    log "INFO" "正在检查GCP项目创建配额..."
    local quota_output
    quota_output=$(gcloud services quota list \
        --service=cloudresourcemanager.googleapis.com \
        --consumer="projects/$(gcloud config get-value project)" \
        --filter="metric.value(name.format()):project_create_requests" \
        --format="value(consumerQuotaLimits.value.effectiveLimit,metric.value.values.currentValue)" 2>/dev/null)

    local limit usage
    read -r limit usage <<< "$quota_output"

    if [[ ! "$limit" =~ ^[0-9]+$ ]]; then
        log "WARN" "无法自动获取配额信息，将跳过检查。"
        read -p "是否继续? [y/N]: " r
        [[ "$r" =~ ^[Yy]$ ]] || return 1
        return 0
    fi
    ...
    log "INFO" "当前项目创建配额: 总计 $limit, 已用 $usage, 剩余 $remaining"

    if (( TOTAL_PROJECTS > remaining )); then
        log "WARN" "计划创建的项目数 ($TOTAL_PROJECTS) 超出剩余配额 ($remaining)!"
        read -p "是否自动将创建数量调整为 $remaining? [Y/n]: " r
        if [[ ! "$r" =~ ^[Nn]$ ]]; then
            TOTAL_PROJECTS=$remaining
            log "INFO" "项目创建数量已调整为: $TOTAL_PROJECTS"
        else
            log "WARN" "将按原计划 ($TOTAL_PROJECTS) 继续，可能因配额问题失败。"
        fi
    fi

    if (( TOTAL_PROJECTS <= 0 )); then
        log "ERROR" "没有足够的配额来创建任何新项目。"
        return 1
    fi
    return 0
}
...
# 其他主要功能函数... (为简洁起见，此处省略，但它们都在下面的完整脚本中)
...
configure_settings() {
    while true; do
        ...
        case $choice in
            1)
                read -p "输入新的项目数量 (1-200): " val
                if [[ "$val" =~ ^[1-9][0-9]*$ && "$val" -le 200 ]]; then
                    TOTAL_PROJECTS=$val
                    log "INFO" "配置更新: 项目数量已设置为 $TOTAL_PROJECTS"
                fi
                ;;
            2)
                read -p "输入新的项目前缀 (小写字母开头,含字母/数字/-): " val
                if [[ "$val" =~ ^[a-z][a-z0-9-]{0,20}$ ]]; then
                    PROJECT_PREFIX="$val"
                    log "INFO" "配置更新: 项目前缀已设置为 $PROJECT_PREFIX"
                fi
                ;;
            3)
                read -p "输入最大并发数 (5-50): " val
                if [[ "$val" =~ ^[0-9]+$ && "$val" -ge 5 && "$val" -le 50 ]]; then
                    MAX_PARALLEL_JOBS=$val
                    log "INFO" "配置更新: 最大并发数已设置为 $MAX_PARALLEL_JOBS"
                fi
                ;;
            4)
                read -p "输入全局等待时间 (建议60-120秒): " val
                if [[ "$val" =~ ^[0-9]+$ && "$val" -ge 30 ]]; then
                    GLOBAL_WAIT_SECONDS=$val
                    log "INFO" "配置更新: 全局等待时间已设置为 $GLOBAL_WAIT_SECONDS"
                fi
                ;;
            5)
                read -p "输入最大重试次数 (1-5): " val
                if [[ "$val" =~ ^[1-5]$ ]]; then
                    MAX_RETRY_ATTEMPTS=$val
                    log "INFO" "配置更新: 最大重试次数已设置为 $MAX_RETRY_ATTEMPTS"
                fi
                ;;
            0) return ;;
            *) echo "无效选项。" && sleep 1 ;;
...
show_menu() {
    ...
    echo "========================================================================"
    echo "  1. 一键创建 ${TOTAL_PROJECTS} 个项目并获取密钥 (推荐, 强健)"
...
check_prerequisites() {
    ...
    if ! gcloud auth list --filter=status:ACTIVE --format="value(account)" | grep -q "."; then
        log "WARN" "未检测到活跃GCP账号，请先使用 'gcloud auth login' 登录。"
        gcloud auth login || return 1
    fi
    if ! command -v jq &>/dev/null; then
        log "WARN" "推荐安装 'jq' 以获得更可靠的JSON解析。脚本将使用内置方法。"
    fi
    log "INFO" "前置检查通过。"
    return 0
}
...
if ! check_prerequisites; then
    log "ERROR" "前置检查失败，程序退出。请解决上述问题后重试。"
    exit 1
fi

log "INFO" "密钥管理器 v3.5 已启动！"
sleep 1
while true; do
    show_menu
done
